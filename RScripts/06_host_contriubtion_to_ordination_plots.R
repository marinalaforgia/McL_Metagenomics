...
samp_data <- as_tibble(ps_gene_lvl@sam_data) %>%
  mutate(AllPlantSpeciesInSubPlot = ifelse(str_detect(AllPlantSpeciesInSubPlot, "LOMU/VUMI"), 'LOMU', as.character(AllPlantSpeciesInSubPlot)),
         ...
         Lasthenia = ifelse(str_detect(AllPlantSpeciesInSubPlot, "Lasthenia"), 1, 0),
  ...
  select(SampleID, Bromus, Avena, Calycadenia, Hemizonia, Elymus,Plantago,Clarkia,Agoseris,Lasthenia,'Festuca perennis','Festuca microstachys') %>%
  arrange(SampleID)
